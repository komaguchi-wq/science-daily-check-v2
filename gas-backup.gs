/**
 * SAPIX理科デイリーチェック v2 用 Google Apps Script バックアップエンドポイント
 *
 * 使い方:
 *   1. https://script.google.com/ で新規プロジェクト作成（社会v2とは別プロジェクトにする）
 *   2. このファイル全体をコピペ
 *   3. SHEET_ID にバックアップ用スプレッドシートのIDを入れる（無ければ自動作成）
 *   4. デプロイ → 新しいデプロイ → 種類「ウェブアプリ」
 *      - 次のユーザーとして実行: 自分
 *      - アクセスできるユーザー: 全員
 *   5. 発行されたURLをアプリの設定画面(⚙️)に貼り付け
 *
 * プロトコル（v2・社会v2と共通）:
...
function getOrCreateSpreadsheet() {
  ...
  const ss = SpreadsheetApp.create("理科デイリーチェック v2 バックアップ");
